...
/* Set when this page was opened as .../certificate.html?regen=<certificates.id>
   from the Certificates list, to redraw an existing certificate rather than
   issuing a new one. The certificate_no / certificate_id / verify_code are
   kept exactly as they were, so the QR code and any link already shared
   keeps working. */
let regenerateId = null;
let regenerateRow = null;
...
/* ---------------- REGENERATE (from Certificates list) ---------------- */

function readRegenerateId() {
  const params = new URLSearchParams(window.location.search);
  return params.get("regen") || null;
}

async function loadRegenerateRow(id) {
  ...
  if (!data) throw new Error("That certificate record was not found.");

  return data;
}

/* Pre-selects institution / student / course and locks the identifying
   numbers so the redraw matches the original certificate exactly.
   NOTE: does its own course loading rather than calling syncStudent(),
   because syncStudent() kicks off loadCourses() without awaiting it,
   which would otherwise wipe out the course selection made here. */
async function applyRegenerateRow(row) {
  if ($("institutionSelect")) $("institutionSelect").value = row.institution_id || "";
  if ($("studentSelect")) $("studentSelect").value = row.student_id || "";
  ...
  await loadCourses(row.institution_id);
  if ($("courseSelect")) $("courseSelect").value = row.course_id || "";
  syncCourse();

  $("certificateNo").value = row.certificate_no || "";
  $("certificateId").value = row.certificate_id || "";
  $("verifyCode").value = row.verify_code || "";
  $("issueDate").value = row.issue_date || todayISO();
  $("dateCompleted").value = row.issue_date || "";
  $("expiryDate").value = row.expiry_date || "";
  $("status").value = row.status || "valid";
  if ($("certificateType")) {
    $("certificateType").value = row.certificate_type || "certificate";
  }

  if ($("formTitle")) $("formTitle").textContent = "Regenerate Certificate";
  const saveBtn = $("saveBtn");
  if (saveBtn) saveBtn.textContent = "Update Certificate";

  showMessage(
    `Regenerating certificate ${row.certificate_no || ""} for ${row.student_name_snapshot || "this student"}. ` +
    `Saving will update the existing record, not create a new one.`,
    "info"
  );
}
...
async function saveCertificate() {
  ...
  // When regenerating, keep the original numbers so the QR / any shared
  // link keeps pointing at the same verify code.
  const certificateNo = regenerateRow?.certificate_no || $("certificateNo").value;
  const certificateId = regenerateRow?.certificate_id || $("certificateId").value;
  const verifyCode = regenerateRow?.verify_code || $("verifyCode").value;
  ...
  if (regenerateId) {
    // Regenerating: update the existing row instead of inserting a new one.
    delete payload.hash_code;   // recomputed below, kept for the new snapshot
    payload.hash_code = hashCode;
  ...
  showMessage(
    regenerateId
      ? `Certificate updated successfully — ${data.certificate_no || certificateNo}`
      : `Certificate saved successfully — ${data.certificate_no || certificateNo}`,
    "success"
  );
  setPreviewStatus(
    regenerateId
      ? "Certificate updated in Supabase successfully."
      : "Certificate saved to Supabase successfully."
  );
...
async function init() {
  try {
    ...
    regenerateId = readRegenerateId();

    if (regenerateId) {
      regenerateRow = await loadRegenerateRow(regenerateId);
      await applyRegenerateRow(regenerateRow);
      setPreviewStatus("Existing certificate loaded. Generate Preview, then Update Certificate.");
    } else {
