...
(() => {
  ...
  function baseVerify(){return `${location.origin}${location.pathname.replace(/Authentication(?:%20| )Letter\.html/i,"")}verify-auth.html?code=${encodeURIComponent(val("verifyCode"))}&id=${encodeURIComponent(val("certificateId"))}`;}
  ...
  function payload(){return{reference_no:val("certificateNo"),authentication_id:val("certificateId"),verification_code:val("verifyCode"),student_id:$("studentSelect")?.value||null,student_name:studentName(),course_id:$("courseSelect")?.value||null,course_name:courseName(),institution_id:$("institutionSelect")?.value||profileInstitution()||null,institution_name:institutionName(),date_started:val("dateStarted")||null,date_completed:val("dateCompleted")||null,issue_date:val("issueDate")||null,expiry_date:val("expiryDate")||null,status:val("status")||"valid",director_name:val("directorName"),academic_head_name:val("academicHeadName"),verification_url:baseVerify(),template_url:"./Authentication Letter.png",student_photo_url:$("studentPhotoPreview")?.src||null,user_id:user?.id||null};}
  async function save(){try{identifiers();if(!$("studentSelect")?.value)throw Error("Student dooro.");if(!$("courseSelect")?.value)throw Error("Course dooro.");await draw();const id=regenId();const p=payload();const r=id?await db.from("authentication_letters").update(p).eq("id",id).select().single():await db.from("authentication_letters").insert(p).select().single();if(r.error)throw r.error;msg(id?"Authentication Letter waa la cusboonaysiiyay.":"Authentication Letter waa la keydiyay.","success");}catch(e){console.error(e);msg(`Save error: ${e.message||e}`,"error");}}
  async function loadExisting(id){const {data,error}=await db.from("authentication_letters").select("*").eq("id",id).single();if(error)throw error;setv("certificateNo",data.reference_no);setv("certificateId",data.authentication_id);setv("verifyCode",data.verification_code);setv("dateStarted",data.date_started);setv("dateCompleted",data.date_completed);setv("issueDate",data.issue_date);setv("expiryDate",data.expiry_date);setv("status",data.status||"valid");setv("directorName",data.director_name);setv("academicHeadName",data.academic_head_name);if(data.institution_id)$("institutionSelect").value=data.institution_id;await students();if(data.student_id)$("studentSelect").value=data.student_id;if(data.course_id)$("courseSelect").value=data.course_id;selectionFields();await draw();msg("Regenerate mode: Authentication Letter waa la furay.","info");}
