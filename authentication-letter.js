...
(() => {
  ...
  const TEMPLATE_CANDIDATES = [
    "./Authentication%20Letter%20v12.png",
  ...
  const L = {
    // Text sits in the former value areas, with no visible input boxes.
    refNo:        {x: 98,  y: 330, w: 156, h: 28},
    issueDate:    {x: 346, y: 330, w: 144, h: 28},
    verifyCode:   {x: 579, y: 330, w: 164, h: 28},
    topStatus:    {x: 838, y: 330, w: 132, h: 28},

    photo:        {x: 57, y: 445, w: 176, h: 210},

    studentName:  {x: 311, y: 456, w: 294, h: 30},
    studentId:    {x: 311, y: 518, w: 294, h: 30},
    courseName:   {x: 311, y: 579, w: 294, h: 30},
    courseCode:   {x: 311, y: 641, w: 294, h: 30},

    dateStarted:  {x: 684, y: 456, w: 289, h: 30},
    dateDone:     {x: 684, y: 518, w: 289, h: 30},
    grade:        {x: 684, y: 579, w: 289, h: 30},
    status:       {x: 684, y: 641, w: 289, h: 30},

    body:         {x: 60, y: 738, w: 935, h: 183},
    authority:    {x: 735, y: 1018, w: 230, h: 30},

    qr:           {x: 78, y: 1176, size: 120},
    verCode:      {x: 357, y: 1166, w: 404, h: 30},
    verStatus:    {x: 357, y: 1225, w: 404, h: 30},
    verUrl:       {x: 357, y: 1281, w: 607, h: 28}
  };
  ...
  function studentPhotoUrl() {
    const s = selectedStudent();
    return s?.photo_url || s?.profile_photo || s?.student_photo_url || s?.avatar_url || s?.photo_path || s?.photo || s?.image_url || s?.image || "";
  }
  ...
  async function resolvePhotoUrl(raw) {
    ...
    // Support Supabase Storage paths such as "bucket/path/to/photo.jpg" or
    // a plain path stored in common student-photo buckets.
    const candidates = [];
    const slash = value.indexOf("/");
    if (slash > 0) candidates.push([value.slice(0, slash), value.slice(slash + 1)]);
    for (const bucket of ["student-photos","student_photos","students","avatars","profiles","photos"]) {
      candidates.push([bucket, value]);
    }
    ...
    for (const [bucket, path] of candidates) {
      try {
        ...
        if (pub) {
          try { await loadImage(pub); return pub; } catch (_) {}
        }
      } catch (_) {}
      try {
        const signed = await db.storage.from(bucket).createSignedUrl(path, 3600);
        if (signed?.data?.signedUrl) {
          try { await loadImage(signed.data.signedUrl); return signed.data.signedUrl; } catch (_) {}
        }
      } catch (_) {}
    }
    return "";
  ...
  async function syncStudent() {
    ...
    if (img && box) {
      const photo = await resolvePhotoUrl(studentPhotoUrl());
      if (photo) { img.src = photo; box.style.display = "block"; }
    }

    if (s?.institution_id && isSuperAdmin()) {
      $("institutionSelect").value = s.institution_id;
      await loadCourses(s.institution_id);
    }

    if (s?.course_id) {
      const exists = courses.some(c => String(c.id) === String(s.course_id));
      if (exists) { $("courseSelect").value = s.course_id; syncCourse(); }
    }

    // If the student has an existing status, use it only when it matches the form.
    const st = String(s?.status || s?.academic_status || "").toLowerCase();
    if (STATUS.includes(st)) $("status").value = st;

    const g = s?.grade || s?.final_grade || s?.result || "";
    if (g && Array.from($("grade")?.options || []).some(o => o.value === String(g))) {
      $("grade").value = String(g);
    }
  }
  ...
  async function draw() {
    ...
    // Student information.
    await drawStudentPhoto(ctx,await resolvePhotoUrl(studentPhotoUrl()));
    ...
    const lines = wrapLines(ctx,bodyText(),L.body.w-30,7);
    const lineH = 22;
    lines.forEach((line,i)=>ctx.fillText(line,L.body.x+15,L.body.y+20+i*lineH));
    ctx.restore();
    ...
    try {
      ...
      // The clean template already has the QR placeholder removed.
      // Draw only the generated QR code, with no extra white overlay.
  ...
  function payload(includeGrade=true) {
    const p = {
      ...
      template_url: "./Authentication%20Letter%20v12.png",
  ...
  function setupEvents() {
    ...
    $("studentSelect")?.addEventListener("change",async()=>{
      await syncStudent();
      generated = false;
      try { await draw(); } catch (e) { msg(`Generate error: ${e.message}`,"error"); }
    });
