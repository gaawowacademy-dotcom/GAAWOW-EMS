/* GAAWOW EMS Authentication Letter Generator v23 - final founder + verification layout */
(() => {
  "use strict";
  ...
  async function drawFounderPhoto(){
    ...
    try{
      ...
      ctx.beginPath(); ctx.arc(103,1042,31,0,Math.PI*2); ctx.clip();
      ...
      ctx.restore();
      ctx.save(); ctx.strokeStyle="#d4a72c"; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(103,1042,31,0,Math.PI*2); ctx.stroke(); ctx.restore();
  ...
  async function draw(){
    ...
    drawEllipsisBoxText(verifyUrl(),355,1280,755,1313,{size:8,min:5,weight:500});
