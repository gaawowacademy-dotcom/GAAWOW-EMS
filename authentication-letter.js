...
(() => {
  ...
  const L = {
    refNo:        {x: 173, y: 337, w: 170, h: 37},
    issueDate:    {x: 422, y: 337, w: 165, h: 37},
    verifyCode:   {x: 670, y: 337, w: 170, h: 37},
    topStatus:    {x: 897, y: 337, w: 160, h: 37},

    photo:        {x: 54, y: 440, w: 188, h: 220},

    studentName:  {x: 310, y: 458, w: 305, h: 37},
    studentId:    {x: 310, y: 519, w: 305, h: 37},
    courseName:   {x: 310, y: 581, w: 305, h: 37},
    courseCode:   {x: 310, y: 642, w: 305, h: 37},

    dateStarted:  {x: 685, y: 458, w: 300, h: 37},
    dateDone:     {x: 685, y: 519, w: 300, h: 37},
    grade:        {x: 685, y: 581, w: 300, h: 37},
    status:       {x: 685, y: 642, w: 300, h: 37},

    // "To Whom It May Concern" body box.
    body:         {x: 60, y: 738, w: 935, h: 183},

    // Official Authentication section.
    authority:    {x: 735, y: 1028, w: 230, h: 30},

    // Verification section.
    qr:           {x: 60, y: 1147, size: 155},
    verCode:      {x: 350, y: 1167, w: 410, h: 34},
    verStatus:    {x: 350, y: 1223, w: 410, h: 34},
    verUrl:       {x: 350, y: 1279, w: 635, h: 34}
  };
  ...
  function courseName() {
    return selectedCourse()?.name || selectedText("courseSelect") || val("courseName");
  }

  function courseCode() {
    return selectedCourse()?.code || selectedCourse()?.course_code || "";
  }
  ...
  async function loadInstitutions() {
    ...
    let q = db.from("institutions").select("id,name").order("name",{ascending:true});
  ...
  async function loadStudents() {
    ...
    let q = db.from("students").select(`
      id,institution_id,student_id,full_name,first_name,last_name,
      photo_url,profile_photo,student_photo_url,avatar_url,
      admission_date,date_started,grade,status,email,phone
    ...
    const {data,error} = await q;
    if (error) {
      // Fallback for installations without grade in students table.
      let q2 = db.from("students").select(`
        id,institution_id,student_id,full_name,first_name,last_name,
        photo_url,profile_photo,student_photo_url,avatar_url,
        admission_date,date_started,status,email,phone
      `).order("full_name",{ascending:true});
      if (iid) q2 = q2.eq("institution_id",iid);
      const r2 = await q2;
      if (r2.error) throw new Error(`Students database error: ${error.message}`);
      students = r2.data || [];
    } else {
      students = data || [];
    }
  ...
  function populateStudents() {
    ...
    students.forEach(r => {
      const o = document.createElement("option");
      o.value = r.id;
      o.textContent = `${r.full_name || `${r.first_name || ""} ${r.last_name || ""}`.trim() || "Unnamed"}${r.student_id ? ` — ${r.student_id}` : ""}`;
  ...
  async function loadCourses(institutionId) {
    ...
    let q = db.from("courses").select("id,institution_id,name,code,course_code,is_active").order("name",{ascending:true});
    ...
    courses.forEach(r => {
      const o = document.createElement("option");
      o.value = r.id;
      o.textContent = r.code || r.course_code ? `${r.name} (${r.code || r.course_code})` : (r.name || r.id);
      s.appendChild(o);
    });
  ...
  function syncStudent() {
    const s = selectedStudent();
    setv("studentName", s?.full_name || `${s?.first_name || ""} ${s?.last_name || ""}`.trim());
  ...
  function syncCourse() {
    ...
    setv("courseName", c?.name || "");
    setv("courseCode", c?.code || c?.course_code || "");
  }
  ...
  async function makeQR(text) {
    ...
    throw new Error("QR library lama helin.");
  }
  ...
  async function draw() {
    ...
    drawBoxText(ctx,val("authorityName") || DEFAULT_AUTHORITY,L.authority,{size:16,weight:"700",align:"center"});
    ...
    drawBoxText(ctx,baseVerifyUrl(),L.verUrl,{size:12,weight:"600",minSize:8});
  ...
  async function save() {
    try {
      ...
      const firstPayload = payload(true);
      result = id
        ? await db.from("certificates").update(firstPayload).eq("id",id).select("*").single()
        : await db.from("certificates").insert(firstPayload).select("*").single();

      // If the existing schema has no grade column, save the rest instead of failing.
      if (result.error && gradeColumnError(result.error)) {
        const fallback = payload(false);
        result = id
          ? await db.from("certificates").update(fallback).eq("id",id).select("*").single()
          : await db.from("certificates").insert(fallback).select("*").single();
        if (!result.error) {
          msg("Authentication Letter waa la keydiyay; grade-ka document-ka ayuu ku jiraa, laakiin database-ka grade column ma laha.","info");
        }
      }
