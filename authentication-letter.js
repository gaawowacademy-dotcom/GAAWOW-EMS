...
(() => {
  ...
  const TEMPLATE = './Authentication Letter v13.png';
  const W = 1055, H = 1491;
  ...
  const G = {
    top: {
      reference: {x: 172, y: 344, max: 150},
      issued: {x: 421, y: 344, max: 145},
      verify: {x: 670, y: 344, max: 150},
      status: {x: 897, y: 344, max: 105}
    },
    student: {
      name: {x: 310, y: 470, max: 290},
      id: {x: 310, y: 531, max: 290},
      course: {x: 310, y: 593, max: 290},
      code: {x: 310, y: 654, max: 290},
      started: {x: 681, y: 470, max: 285},
      completed: {x: 681, y: 531, max: 285},
      grade: {x: 681, y: 593, max: 285},
      status: {x: 681, y: 654, max: 285}
    },
    photo: {x: 57, y: 444, w: 178, h: 210},
    body: {x: 62, y: 748, w: 930, maxLines: 5, line: 28},
    verification: {
      qr: {x: 77, y: 1154, size: 118},
      code: {x: 355, y: 1181, max: 390},
      status: {x: 355, y: 1237, max: 390},
      url: {x: 355, y: 1288, max: 600}
    }
  };
  ...
  async function draw(){if(!ctx)throw Error('Canvas lama helin.');ctx.clearRect(0,0,W,H);ctx.drawImage(template,0,0,W,H);
    drawValue(val('certificateNo'),172,344,150,{size:14,min:9,weight:700});
    drawValue(fmtDate(val('issueDate')),421,344,145,{size:14,min:9,weight:600});
    drawValue(val('verifyCode'),670,344,150,{size:14,min:9,weight:700});
    drawValue((val('status')||'VALID').toUpperCase(),897,344,105,{size:14,min:9,weight:700});
    drawValue(studentName(),310,470,290,{size:17,min:10,weight:600});
    drawValue(val('studentIdPreview'),310,531,290,{size:17,min:10,weight:600});
    drawValue(courseName(),310,593,290,{size:17,min:10,weight:600});
    drawValue(val('courseCode'),310,654,290,{size:17,min:10,weight:600});
    drawValue(fmtDate(val('dateStarted')),681,470,285,{size:17,min:10,weight:600});
    drawValue(fmtDate(val('dateCompleted')),681,531,285,{size:17,min:10,weight:600});
    drawValue(val('grade'),681,593,285,{size:17,min:10,weight:700});
    drawValue((val('status')||'VALID').toUpperCase(),681,654,285,{size:17,min:10,weight:600});
    drawBody();
    ...
    const u=baseVerify();try{if(window.QRCode?.toDataURL){const data=await new Promise((res,rej)=>QRCode.toDataURL(u,{width:240,margin:0,errorCorrectionLevel:'M'},(err,url)=>err?rej(err):res(url)));const qi=await loadImage(data);ctx.drawImage(qi,G.verification.qr.x,G.verification.qr.y,G.verification.qr.size,G.verification.qr.size);}}catch(e){console.warn('QR render failed',e);}
    drawValue(val('verifyCode'),355,1181,390,{size:15,min:10,weight:700});
    drawValue((val('status')||'VALID').toUpperCase(),355,1237,390,{size:15,min:10,weight:700});
    drawValue(u,355,1288,600,{size:8,min:6,weight:500,color:'#344054'});
    setStatus('Authentication Letter preview ready.');
  }

  async function payload(){const core=`${val('certificateId')}|${val('certificateNo')}|${val('verifyCode')}|${$('studentSelect')?.value||''}|${$('courseSelect')?.value||''}|${val('issueDate')||''}`;const hash=await sha256(core);return{certificate_no:val('certificateNo'),certificate_id:val('certificateId'),verify_code:val('verifyCode'),hash_code:hash,student_id:$('studentSelect')?.value||null,student_name:studentName(),course_id:$('courseSelect')?.value||null,course_name:courseName(),institution_id:$('institutionSelect')?.value||profileInstitution()||null,institution_name:institutionName(),date_started:val('dateStarted')||null,date_completed:val('dateCompleted')||null,issue_date:val('issueDate')||null,expiry_date:val('expiryDate')||null,status:val('status')||'valid',certificate_type:'authentication_letter',director_name:val('directorName')||'Dr. Osmaan Mohamed Ibraahin',verification_url:baseVerify(),template_url:TEMPLATE,user_id:user?.id||null};}
  async function save(){try{identifiers();if(!$('studentSelect')?.value)throw Error('Student dooro.');if(!$('courseSelect')?.value)throw Error('Course dooro.');await draw();const p=await payload();const id=regenId();let r=id?await db.from('certificates').update(p).eq('id',id).select().single():await db.from('certificates').insert(p).select().single();if(r.error)throw r.error;msg(id?'Authentication Letter waa la cusboonaysiiyay.':'Authentication Letter waa la keydiyay.','success');}catch(e){console.error(e);msg(`Save error: ${e.message||e}`,'error');}}
