...
(() => {
  ...
  async function draw(){if(!ctx)throw Error('Canvas lama helin.');ctx.clearRect(0,0,W,H);ctx.drawImage(template,0,0,W,H);
    ...
    drawValue(studentName(),320,470,280,{size:17,min:10,weight:600});
    drawValue(val('studentIdPreview'),320,531,280,{size:17,min:10,weight:600});
    drawValue(courseName(),320,593,280,{size:17,min:10,weight:600});
    drawValue(val('courseCode'),320,654,280,{size:17,min:10,weight:600});
    drawValue(fmtDate(val('dateStarted')),691,470,275,{size:17,min:10,weight:600});
    drawValue(fmtDate(val('dateCompleted')),691,531,275,{size:17,min:10,weight:600});
    drawValue(val('grade'),691,593,275,{size:17,min:10,weight:700});
    drawValue((val('status')||'VALID').toUpperCase(),691,654,275,{size:17,min:10,weight:600});
    drawBody();
    ...
    drawValue(val('verifyCode'),365,1181,380,{size:15,min:10,weight:700});
    drawValue((val('status')||'VALID').toUpperCase(),365,1237,380,{size:15,min:10,weight:700});
